<?php
    ...
    class sms_connection
    {
         ...
         *  @param  $unicode        Whether the message is unicode encoded or not. If the message
         *                          is unicode encoded, we expect the $message parameter to contain
         *                          UCS-2 encoded characters
         *
         ...
         */
        function send_one_way_sms($message, $mobiles, $senderid, $max_segments = 1, $unicode = false)
        {
            ...
            $url =  $this->server_url . URI_SEND_MESSAGE . "?connectionid=" . $this->connectionid;
            $url .= "&type=" . MESSAGE_TYPE_1WAY;
            $url .= "&senderid=" . urlencode($senderid);
            $url .= "&to=" . $this->array_to_string($mobiles);
            $url .= "&max_segments=$max_segments";

            if($unicode)
            {
                // Ensure the message is in UCS-2 and then convert it into Hexadecimal digits
                $ucs2 = mb_convert_encoding($message, "UCS-2", "auto");
                $hex  = bin2hex($ucs2);
                $url .= "&message=" . $hex;
                $url .= "&unicode=true";
            }
         ...
         *                          Otherwise, the ID of the SMS sent is returned as a string
         */
        function send_two_way_sms($message, $mobiles, $messageid = null, $max_segments = 1, $unicode = false)
        {
            if(!$this->is_connected())
            {
                return null;
